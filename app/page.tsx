...
import type { MapLayerMouseEvent } from "mapbox-gl"; // 타입 전용 임포트(런타임 포함 안 됨)
...
export default function Home() {
  ...
  // 브라우저 GPS를 Promise로 래핑(GeolocationPosition 타입에 의존하지 않게 단순화)
  ...
  const locateMe = async () => {
    ...
    setLocating(true);
    try {
      const pos = await getBrowserPosition().catch(async () => await ipFallback());
      if (pos) flyTo(pos);
    } finally {
      setLocating(false);
    }
  ...
  useEffect(() => {
    ...
    map.on("load", async () => {
      try {
        ...
        // 클릭 → 팝업 (타입 안전 + any 미사용)
        map.on("click", "places-layer", (e: MapLayerMouseEvent) => {
          const raw = e.features?.[0];
          if (!raw) return;

          // GeoJSON Feature 형태로 안전 캐스팅
          const feat = raw as unknown as PlaceFeature;
